<template>
	<view class="home">
		...
		<view class="content">
			<!-- 			AI总结 -->
			<view class="aiSumup" v-if="readNums!=0">
				<view class="typing-text" ref="typingText">
					<text user-select>{{ typedText }}</text>
				</view>
				<view class="expand-collapse-button" @click="showHideNewArrs">
					<view class="readNums">
						<text>包含{{readNums}}条检索信息-{{ isExpanded ? '展开' : '收起' }}</text>
					</view>
				</view>
			</view>


			<view class="row" v-for="item in noNewsArr" :key="item.id">
	...
	export default {
		...
		methods: {
			...
			handleSearch() {
				...
				// 	}
				// });
				this.readNums = this.newsArr.length
				console.log("测试数组长度" + this.readNums)
			},
			...
			showHideNewArrs() {
				//默认true收起,新闻数组置空
				if (this.isExpanded) {
					const temp = this.newsArr
					this.newsArr = this.noNewsArr
					this.noNewsArr = temp
				} else {
					//恢复,no里面有数据
					const temp = this.noNewsArr
					this.noNewsArr = this.newsArr
					this.newsArr = temp
				}
				this.isExpanded = !this.isExpanded
